...
Вычисляем хеш по ключу переданного элемента:
   1. Если ячейка по данному хешу уже инициализирована, запускаем операцию поиска, чтобы проверить ключ на уникальность. Если ключ оказался уникальным, то вставлям переданный элемент в начало списка и завершаем работу, если нет, то также завершаем работу алгоритма, но без добавления элемента.
